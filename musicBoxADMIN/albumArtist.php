<?php include("includesArtist/includedFiles.php"); 
...
<div class="entityInfoAlbum">
	...
	<div class="leftSectionAlbum">
		<img src="artworkAlbum/<?php echo $album->getArtworkPath();?>">
	</div>
	...
	<div class="rightSection">
		...
		<p class="AlbumText">Виконавець: <span class = "AlbumAuthor" onclick="openPage('artistsArtist.php?id=<?php echo $artistId; ?>')"><?php echo $artist->getName();?></span></p>
		...
		<?php 
		$artistOwner = $userLoggedIn->getUserId();
		$albumOwner = $artistId;
		if($artistOwner == $albumOwner){
			echo "<span class='albumButtonAdd'><img title='Видалити альбом'  onclick='deleteAlbum($albumId)' src='assets\images\icons\deleteWhite.png'></span>";
		}?>
	...
	<ul class="tracklist">
		...
		foreach($songIdArray as $songId) {
			...
			echo "<li class='tracklistRow'>
					<div class='trackCount'>
						<img class='play' onclick='setTrack(\"" . $albumSong->getId() . "\", tempPlaylist, true)'>
...
<?php
	...
	if(mysqli_num_rows($albumQuery) > 1) {
		echo "<h2>Інші релізи від: <span class = 'ArtistHover' onclick='openPage(\"artistArtist.php?id=" . $artistId . "\")'>" . $artist->getName() . "</span></h2>";
	}
		...
		while($row = mysqli_fetch_array($albumQuery)) {
			if($row['title'] != $album->getTitle()){
				echo "<div class='gridViewItem'>
					<span role='link' tabindex='0' onclick='openPage(\"albumArtist.php?id=" . $row['id'] . "\")'>
						<img src='artworkAlbum/" . $row['artworkPath'] . "'>
