...
?>
	...
	<div class="leftSection">
		...
		<img src="musicBoxADMIN/artworkArtist/<?php echo $artist->getArtistPic();?>">
	...
	<?php 
		...
		$user = $userLoggedIn->getUserId();
		$artistId = $artist->getId();
		$FollowCheck = mysqli_query($con, "SELECT * FROM follows WHERE userId = '$user' AND artistId = '$artistId' ");
		if(mysqli_num_rows($FollowCheck) == 1) {
			$status = "Підписка";
		}
		else $status = "Підписатися";
		$artistId = $artist->getId();
		echo "<button class='followButton' onclick='newFollow($artistId)'>$status</button>";
	...
	foreach($albumIdArray as $albumId) {
			...
			echo "<div class='gridViewItem'>
				<span role='link' tabindex='0' onclick='openPage(\"album.php?id=" . $ThisAlbum->getId() . "\")'>
					<img src='musicBoxADMIN/artworkAlbum/" . $ThisAlbum->getArtworkPath() . "'>
